        ...
        <div class="form-group">
            <form method="post" action="index.php" novalidate>
                ...
                <div class="birthfields">
                    <label for="birthdate">Date de naissance : </label><span class="errormessage"><?= isset($_POST['birthdate']) ? (empty($_POST['birthdate']) ? 'Veuillez remplir ce champ' : (preg_match($dateRegex, $birthDate) == false ?  "Veuillez indiquer une date de naissance valide" : "")) : "" ?></span>
                    <input type="date" name="birthdate" id="birthdate" value="<?= isset($_POST['birthdate']) ? $_POST['birthdate'] : '' ?>" required>
                </div>
                <!-- NATIONALITE -->
                <div class="textfields">
                    <label for="nationality">Nationalité: </label><span class="errormessage"><?= isset($_POST['nationality']) ? (empty($_POST['nationality']) ? 'Veuillez remplir ce champ' : (preg_match($lastNameRegex, $nationality) == false ?  "Veuillez indiquer une nationalité valide" : "")) : "" ?></span>
                    <input type="text" name="nationality" id="nationality" value="<?= isset($_POST['nationality']) ? $_POST['nationality'] : '' ?>" required>
                </div>
                <!-- ADRESSE -->
                <div class="adressfields">
                    <label for="adress">Adresse : </label><span class="errormessage"><?= isset($_POST['adress']) ? (empty($_POST['adress']) ? 'Veuillez remplir ce champ' : (preg_match($addressRegex, $adress) == false ?  "Veuillez indiquer une adresse valide" : "")) : "" ?></span>
                    <input type="text" name="adress" id="adress" value="<?= isset($_POST['adress']) ? $_POST['adress'] : '' ?>" required>
                </div>
                <!-- EMAIL -->
                ...
                <div class="phonefields">
                    <label for="phonenumber">Numéro de téléphone : </label><span class="errormessage"><?= isset($_POST['phonenumber']) ? (empty($_POST['phonenumber']) ? 'Veuillez remplir ce champ' : (preg_match($phoneRegex, $phoneNumber) == false ?  "Veuillez indiquer un numéro de téléphone valide" : "")) : "" ?></span>
                    <input type="tel" name="phonenumber" id="phonenumber" value="<?= isset($_POST['phonenumber']) ? $_POST['phonenumber'] : '' ?>" required>
                </div>
                <!-- DIPLOME -->
                <div class="fields">
                    <label for="degree">Diplômes : </label><span class="errormessage"><?= isset($_POST['degree']) ? (empty($_POST['degree']) ? 'Veuillez remplir ce champ' : (preg_match($degreeRegex, $degree) == false ?  "Veuillez indiquer un diplôme valide" : "")) : "" ?></span>
                    <select name="degree" id="degree"  required>
                        <option selected>--Choississez un diplôme--</option>
                        <option value="1" <?= (isset($_POST['degree']) && $_POST['degree'] === '1') ? 'selected' : ''; ?>>Sans diplômes</option>
                        ...
                        <option value="4" <?= (isset($_POST['degree']) && $_POST['degree'] === '4') ? 'selected' : ''; ?>>Bac +3 et supérieur</option>

                    </select>
                </div>
                <!-- NUMERO POLE EMPLOI -->
                <div class="poleemploifields">
                    <label for="poleemploinumber">Numéro Pôle Emploi : </label><span class="errormessage"><?= isset($_POST['poleemploinumber']) ? (empty($_POST['poleemploinumber']) ? 'Veuillez remplir ce champ' : (preg_match($poleEmploiRegex, $poleEmploi) == false ?  "Veuillez indiquer un numéro de Pôle Emploi valide" : "")) : "" ?></span>
                    <input type="text" name="poleemploinumber" id="poleemploinumber" value="<?= isset($_POST['poleemploinumber']) ? $_POST['poleemploinumber'] : '' ?>" required>
                </div>
                <!-- NOMBRE DE BADGE -->
                <div class="numberfields">
                    <label for="badgeamount">Nombre de badge : </label><span class="errormessage"><?= isset($_POST['badgeamount']) ? (empty($_POST['badgeamount']) ? 'Veuillez remplir ce champ' : (preg_match($badgesRegex, $badges) == false ?  "Veuillez indiquer un nombre de badge valide" : "")) : "" ?></span>
                    <input type="text" name="badgeamount" id="badgeamount" value="<?= isset($_POST['badgeamount']) ? $_POST['badgeamount'] : '' ?>" required>
                </div>
                <!-- Liens codeacademy -->
                <div class="academyfields">
                    <label for="links">Lien codeacademy : </label><span class="errormessage"><?= isset($_POST['links']) ? (empty($_POST['links']) ? 'Veuillez remplir ce champ' : (preg_match($urlRegex, $codecademyLinks) == false ?  "Veuillez indiquer un lien codeacademy valide" : "")) : "" ?></span>
                    <input type="text" name="links" id="links" value="<?= isset($_POST['links']) ? $_POST['links'] : '' ?>" required>
                </div>
                <!-- SUPER HEROS -->
                <div class="textareafields">
                    <label for="superheroes">Si vous étiez un super héros/une super héroïne, qui seriez-vous et pourquoi?</label><span class="errormessage"><?= isset($_POST['superheroes']) ? (empty($_POST['superheroes']) ? 'Veuillez remplir ce champ' : "") : "" ; ?></span>
                    <textarea id="superheroes" name="superheroes" rows="1" cols="15" placeholder="Votre super héros favori?" required><?= isset($_POST['superheroes']) ? $_POST['superheroes'] : '' ?></textarea>
                </div>
                <!-- HACK -->
                <div class="textareafields">
                    <label for="hack">Racontez-nous un de vos "hacks" (pas forcément technique ou informatique)</label><span class="errormessage"><?= isset($_POST['hack']) ? (empty($_POST['hack']) ? 'Veuillez remplir ce champ' : "") : "" ; ?></span>
                    <textarea id="hack" name="hack" rows="1" cols="15" placeholder="Racontez votre hack"  required><?= isset($_POST['hack']) ? $_POST['hack'] : '' ?></textarea>
                </div>
